...
from PySide6.QtWidgets import QGraphicsScene, QGraphicsPixmapItem, QFileDialog, QSizePolicy, QSpacerItem, QPushButton
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("PowerSynth MOSTCOOL GUI - 2025.06")
        self.cpu_rows = []
        self.gpu_rows = []
        self.psu_rows = []
        self.cpu_table = None
    ...
    def createRunPSCase(self):
        ...
        compName, hsFins = createPartFile(self.data, workFolder)
        ...
        #if ExitCode == 0:
        self.showResultsWindow(workFolder, compName, hsFins)
    ...
    def showResultsWindow(self, workFolder, compName, hsFins):
        ...
        self.figure2 = plt.Figure(figsize=(16, 12), dpi=100)   # , constrained_layout=True
        ...
        def loadRegionData(path, region, latestTime):
            try:
                ...
                if len(np.unique(z)) > 1:
                    if region == 'pcb':
                        ...
                        #print((self.data['cpus'][0]))                        
                        mask = z > thick - 2  # Slice at min(z); adjust if needed
                    
                    start = abs(len(temperature) - len(x)) 
                    if start != 0:
                        temperature = temperature[:-start]    
                    x, y, temperature = x[mask], y[mask], temperature[mask]- 273.15
                else:
                    x, y, temperature = x, y, temperature
                return region, x, y, temperature, None
            except Exception as e:
                print(f"Error loading region {region}: {e}")
                return region, None, None, None, str(e)
        
        # Create partial function for threading
        read_data = partial(loadRegionData, path, latestTime=latestTime)
        
        # Read all regions in parallel
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(int(self.cpuCount), 1)) as executor:  # len(regions)
            results = list(executor.map(read_data, [r for r in regions]))

        # Process results
        region_data = {}
        for result in results:
            region, x, y, temperature, error = result
            if error:
                print(f"Error reading region {region}: {error}")
                continue
            region_data[region] = (x, y, temperature)
        ...
        self.toolbar = CustomNavigationToolbar(self.canvas, self.solution_window, region_data, self.plots, ax)
        ...
        for region in regions:
            if region == 'air':
                ...
                region_grids[region] = (int(self.data['define_server']['pcb']['x']/8), int(self.data['define_server']['pcb']['y']/8), 2)
                region_sizes[region] = 20
            elif region[1:3] == 'pu':
                region_grids[region] = (12, 12, 1)
                region_sizes[region] = 10
            elif region[1:3] == 'su':
                region_grids[region] = (20, 10, 10)
                region_sizes[region] = 10
            else:
                index = int(region[-1]) - 1 
                region_grids[region] = (20, hsFins[index], 10)
                region_sizes[region] = 10
        ...
        grouped_regions3d = {"Heatsinks": HS, "Component": compName[1:], "PCB": compName[0:1]}
        regions3d = compName + HS
        self.plots3D = {}
        # Create shared normalization and colormap
        cmap = cm.rainbow
        norm3d = Normalize()
        sm3d = cm.ScalarMappable(norm=norm3d, cmap=cmap)
        sm3d.set_array(all_temps)  # Set array with all possible values
        
        all_temperatures = []
        # Function to read data for a single region
        def read_region_data(path, region, latestTime):
            try:
                #print(region)
                x, y, z = ff.readmesh(path, region=region, precision=3, verbose=False)
                temperature = ff.readfield(path, time_name=latestTime, name='T', region=region, precision=3, verbose=False)
                return region, x, y, z, temperature, None
            except Exception as e:
                return region, None, None, None, None, str(e)

        # Create partial function for threading
        read_func = partial(read_region_data, path, latestTime=latestTime)
        
        # Read all regions in parallel
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(int(self.cpuCount), 1)) as executor:
            results = list(executor.map(read_func, [r for r in regions if r not in ['air', 'uniform']]))

        # Process results
        region_data = {}
        for result in results:
            region, x, y, z, temperature, error = result
            if error:
                print(f"Error reading region {region}: {error}")
                continue
            region_data[region] = (x.astype(np.float64), y.astype(np.float64), z.astype(np.float64), temperature.astype(np.float64))
            
        resultsStr = {}
        # Process each region's data
        for region, (x, y, z, temperature) in region_data.items():
            try:
                # Convert units and handle data
                x_uns = x * 1000
                y_uns = y * 1000
                z_uns = z * 1000
                t_uns = temperature - 273.15  # K to C
                
                # Align array lengths
                min_len = min(len(x_uns), len(y_uns), len(z_uns), len(t_uns))
                x_uns = x_uns[:min_len]
                y_uns = y_uns[:min_len]
                z_uns = z_uns[:min_len]
                t_uns = t_uns[:min_len]
                
                all_temperatures.extend(t_uns)
                
                # Create structured grid
                nx, ny, nz = region_grids[region]
                x_min, x_max = np.min(x_uns), np.max(x_uns)
                y_min, y_max = np.min(y_uns), np.max(y_uns)
                z_min, z_max = np.min(z_uns), np.max(z_uns)
                x_str = np.linspace(x_min, x_max, nx, dtype=np.int16)
                y_str = np.linspace(y_min, y_max, ny, dtype=np.int16)
                z_str = np.linspace(z_min, z_max, nz, dtype=np.int16)
                X_str, Y_str, Z_str = np.meshgrid(x_str, y_str, z_str, indexing='ij')
                
                # Interpolate data
                points_uns = np.column_stack((x_uns, y_uns, z_uns))
                T_str = griddata(points_uns, t_uns, (X_str, Y_str, Z_str), method='nearest')
                T_str = np.nan_to_num(T_str, nan=np.nanmean(T_str))
                T_str = T_str.astype(np.int16)
                if region == 'pcb':
                    X_str = X_str[:, :, 1:2]
                    Y_str = Y_str[:, :, 1:2]
                    Z_str = Z_str[:, :, 1:2]
                ...
                global_t_min = np.nanmin(all_temperatures)
                global_t_max = np.nanmax(all_temperatures)
                
                locals()[region] = {'Coord': [X_str.ravel().tolist(), Y_str.ravel().tolist(), Z_str.ravel().tolist()], 'T': T_str.ravel().tolist()}
                resultsStr[region] = locals()[region]
            
                scatter = ax2.scatter(X_str, Y_str, Z_str, c=T_str, marker='s', cmap=cmap, vmin=default_min, vmax=default_max, s=region_sizes[region])
                scatter.set_visible(True)  # Initially Visible
                self.plots3D[region] = scatter
        ...
        with open(os.path.join(resultFolder, 'resultsStru.json'), 'w') as f:
            json.dump(resultsStr, f)
        
        # Create colorbar
        cbar3d = self.figure2.colorbar(sm3d, ax=ax2, shrink=0.5, pad=0.15)
        cbar3d.set_label('Temperature (C)', fontsize=10, labelpad=10)
        
        # Customize axes and title
        # Axis labels and title
        ax2.set_xlabel('X [mm]', labelpad=10)
        ax2.set_ylabel('Y [mm]', labelpad=4)
        ax2.set_zlabel('Z [mm]', labelpad=2)
        ...
        def toggleRegion3D():
            for label, checkbox in self.checkboxes3D.items():
                if checkbox.isChecked():
                    if label in grouped_regions3d:
                        for region in grouped_regions3d[label]:
                            if region in self.plots3D:
                                self.plots3D[region].set_visible(True)
                    else:
                        if label in self.plots3D:
                            self.plots3D[label].set_visible(True)
                else:
                    if label in grouped_regions3d:
                        for region in grouped_regions3d[label]:
                            if region in self.plots3D:
                                self.plots3D[region].set_visible(False)
                    else:
                        if label in self.plots3D:
                            self.plots3D[label].set_visible(False)
            update3dPlot()
            #self.canvas2.draw()
            
        def update3dPlot():
            visible_plots = [p for p in self.plots3D.values() if p.get_visible()]
            if visible_plots:
                try:
                    t_min = float(self.min_edit3D.text()) if self.min_edit3D.text().strip() else norm3d.vmin
                    t_max = float(self.max_edit3D.text()) if self.max_edit3D.text().strip() else norm3d.vmax
                    if t_min >= t_max:
                        raise ValueError("Min must be less than Max")
                except ValueError as e:
                    print(f"Invalid input: {e}. Using previous range: {norm3d.vmin} - {norm3d.vmax}")
                    t_min, t_max = norm3d.vmin, norm3d.vmax
        ...
        checkboxes_layout = QHBoxLayout(checkboxes_widget)
        self.checkboxes3D = {}
        toggle_options = list(grouped_regions3d.keys()) + [r for r in regions3d if not any(r in v for v in grouped_regions3d.values())]
        for label in toggle_options:
            checkbox = QCheckBox(label)
            checkbox.setChecked(True)
            checkbox.stateChanged.connect(toggleRegion3D)
            self.checkboxes3D[label] = checkbox
            checkboxes_layout.addWidget(checkbox)
        ...
        temp_limits_layout = QHBoxLayout(temp_limits_widget)
        temp_limits_layout.addWidget(QLabel("Min T (C):"))
        self.min_edit3D = QLineEdit(f"{global_t_min:.2f}")
        self.min_edit3D.setFixedWidth(40)
        validator = QDoubleValidator()
        validator.setNotation(QDoubleValidator.StandardNotation)
        self.min_edit3D.setValidator(validator)
        self.min_edit3D.returnPressed.connect(updateTempRange3D)
        temp_limits_layout.addWidget(self.min_edit3D)

        temp_limits_layout.addWidget(QLabel("Max T (C):"))
        self.max_edit3D = QLineEdit(f"{global_t_max:.2f}")
        self.max_edit3D.setFixedWidth(40)
        self.max_edit3D.setValidator(validator)
        self.max_edit3D.returnPressed.connect(updateTempRange3D)
        temp_limits_layout.addWidget(self.max_edit3D)
        
        # Top horizontal layout for toolbar and controls
        top_layout = QHBoxLayout()

        self.toolbar3 = CustomNavigationToolbar(self.canvas2, self.solution_window, region_data, self.plots3D, ax2)
        
        ax2.format_coord = lambda x, y: ""
        
        top_layout.addWidget(self.toolbar3, stretch=1)  # Toolbar expands
        # Outer vertical line separator
        separator_outer = QFrame()
        separator_outer.setFrameShape(QFrame.VLine)
        separator_outer.setFrameShadow(QFrame.Sunken)
        ...
        # Add to Tab 2's main vertical layout
        self.solution_ui.verticalLayout.addLayout(top_layout)
        self.solution_ui.verticalLayout.addWidget(self.canvas2, stretch=1)  # Canvas expands vertically
        
        self.solution_ui.tabWidget.setTabVisible(2, False)  # Disable 3D PNG tab
        
        # tab 3 Summary
        self.resultsPath = QLabel(" ")
        ...
        self.resultsPath.setText(f"Results Folder: {resultFolder}")
        self.solution_ui.verticalLayout_5.addWidget(self.resultsPath)
...
def createPartFile(data, workFolder):
    ...
    for components in data["Components"].values():
        for comp in components:
            ...
            if 'heatsink' in comp:
                if comp['heatsink']:
                    hsFins.append(comp['heatsink']['n_fins'])
            name = comp['id'] + '.part'
...
class CustomNavigationToolbar(NavigationToolbar):
    ...
    def __init__(self, canvas, parent, region_data, plots, ax):
        ...
        if 'hs1' in self.plots:
            
            def TopView():
                self.ax.view_init(elev=90, azim=-90)
                self.canvas.draw_idle() # Redraw the figure to apply the changes
            ...
            # Create buttons
            self.buttonTop = QPushButton("Top View")
            #self.buttonSide = QPushButton("Side View")
            self.buttonTop.clicked.connect(TopView)
            #self.buttonSide.clicked.connect(SideView)
            
            # Add separator and buttons to toolbar
            self.addSeparator()
            self.addWidget(self.buttonTop)
            #self.addSeparator()
            #self.addWidget(self.buttonSide)
    ...
    def home(self, *args, **kwargs):
            if 'hs1' in self.plots:
                """Restore home view, including 3D angles if stored."""
                super().home(*args, **kwargs)  # Restore 2D view (xlim, ylim)
